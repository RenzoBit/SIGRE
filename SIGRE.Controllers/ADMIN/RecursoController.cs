...
namespace SIGRE.Controllers.ADMIN
{
	...
	public class RecursoController : Controller
	{
		...
		{
			IQueryable<Recurso> recurso;
			bool ignora = false;
			if (sesion == 0 || Session["p_nombre"] == null)
				ignora = true;
			if (ignora)
			{
				Session["p_nombre"] = null;
				recurso = db.Recurso.Include(r => r.propietario);
			}
			else
			{
				string nombre = (string)Session["p_nombre"];
				recurso = db.Recurso.Where(
				s => (nombre.Trim().Equals("") || s.nombre.Contains(nombre.Trim()))
		...
		public ActionResult Index(string nombre = "")
		{
			...
			var o = db.Recurso.Where(
				s => (nombre.Trim().Equals("") || s.nombre.Contains(nombre.Trim()))
				).Include(r => r.propietario);
			List<Recurso> l = o.ToList();
			if (l.Count() == 0)
		...
		public ActionResult IndexAprobar()
		{
			...
			List<Recurso> l = db.Recurso.Where(x => x.idcolaborador == colaborador.idcolaborador && !x.desactivado && x.aprobado == null).Include(x => x.propietario).ToList();
			return View(l);
		}
		...
		public ActionResult EditAprobar(int id = 0)
		{
			...
			recurso.propietario_nombre = recurso.propietario.nombre;
			return View(recurso);
		}
		...
		{
			Recurso recurso = db.Recurso.Find(id >= 0 ? id : -id);
			recurso.aprobado = (id >= 0);
			db.Entry(recurso).State = EntityState.Modified;
			db.Entry(recurso).Property("foto").IsModified = false;
			db.SaveChanges();
			Colaborador rolando = db.Colaborador.Where(x => x.usuario.idrol == Rol.CONSULTOR && x.aprobado == true && !x.desactivado).FirstOrDefault();
		...
		public ActionResult Edit(int id = 0)
		{
			Recurso recurso = db.Recurso.Find(id);
			recurso.propietario_nombre = recurso.propietario.nombre;
			//recurso.idcentrocosto = recurso.propietario.centrocosto.centrocosto_idcentrocosto ?? default(int);
			if (recurso == null)
			{
				return HttpNotFound();
			}
		...
		public ActionResult Edit2(int id = 0)
		{
			...
			if (recurso == null)
			{
				TempData["alerta"] = "El recurso fue eliminado";
				return RedirectToAction("Index", "Home", new { Area = "" });
			}
			else if (recurso.aprobado == true)
			{
				TempData["alerta"] = "El recurso ya fue aprobado";
				return RedirectToAction("Index", "Home", new { Area = "" });
			}
			else if (recurso.aprobado == false)
			{
				TempData["alerta"] = "El recurso ya fue rechazado";
				return RedirectToAction("Index", "Home", new { Area = "" });
			}
			//ViewBag.idcentrocosto = new SelectList(db.CentroCosto.Where(c => c.centrocosto_idcentrocosto == null), "idcentrocosto", "nombre", recurso.idcentrocosto);
			return View(recurso);
		...
		public ActionResult Edit(Recurso recurso)
		{
			...
			if (ModelState.IsValid)
			{
				recurso.aprobado = null;
				db.Entry(recurso).State = EntityState.Modified;
				db.Entry(recurso).Property("foto").IsModified = cargafoto;
				db.SaveChanges();
				TempData["alerta"] = "Solicitud de validación enviada con éxito";
				string correo = db.Colaborador.Find(recurso.idcolaborador).correo;
				string titulo = "Solicitud de validación";
				string mensaje = "Ud. tiene una solicitud de validación de recurso<br/><br/><a href=\""
					+ (Request.Url.Scheme + "://" + Request.Url.Authority + Request.ApplicationPath.TrimEnd('/') + "/ADMIN/Recurso/Edit2/" + recurso.idrecurso)
					+ "\" target=\"_blank\">IR</a>";
				if (Correo.sepuede)
			...
			//ViewBag.idcolaborador = new SelectList(db.Colaborador, "idcolaborador", "nombre", recurso.idcolaborador);
			//ViewBag.colaborador_idcolaborador = new SelectList(db.Colaborador, "idcolaborador", "nombre", recurso.colaborador_idcolaborador);
			return View(recurso);
		}
		...
		[ValidateAntiForgeryToken]
		[HandleError]
		public ActionResult Edit2(Recurso recurso)
		{
			db.Entry(recurso).State = EntityState.Modified;
			db.Entry(recurso).Property("foto").IsModified = false;
			db.SaveChanges();
		...
		public ActionResult Delete(int id = 0)
		{
			...
			if (id > 0)
			{
				List<PerfilRecurso> l = db.PerfilRecurso.Where(c => c.idrecurso == id).ToList();
				foreach (PerfilRecurso o in l)
				{
					db.PerfilRecurso.Remove(o);
					db.SaveChanges();
				}
				mensaje = "Su recurso " + recurso.nombre + " fue eliminado.";
				TempData["alerta"] = "El recurso fue eliminado satisfactoriamente";
				db.Recurso.Remove(recurso);
			}
			else
			{
				mensaje = "Su recurso " + recurso.nombre + " fue desactivado.";
				TempData["alerta"] = "El recurso fue desactivado satisfactoriamente";
				recurso.desactivado = true;
				db.Entry(recurso).State = EntityState.Modified;
			}
			db.SaveChanges();

			string correo = db.Colaborador.Find(recurso.idcolaborador).correo;
			string titulo = "Actualización de recurso";
			if (Correo.sepuede)
				Correo.enviar2(correo, titulo, mensaje);
			return RedirectToAction("Index");
		}
		...
		{
			byte[] foto = (id == 0 ? null : db.Recurso.Find(id).foto);
			if (foto == null)
			{
				foto = new byte[7118];
